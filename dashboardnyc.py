...
page = st.sidebar.radio("Go to", ["Overview", "Flight Route Statistics", "Delay Analysis", "Time-based Statistics"])
...
if page == "Overview":
    ...
    with col2:
        ...
        query_departure_airport = "SELECT origin, COUNT(*) as flight_count FROM flights GROUP BY origin"
    ...
    with col_delay_breakdown:
     ...
     df_delays['delay_category'] = np.select(conditions, categories, default='Unknown')
     delay_cat = df_delays['delay_category'].value_counts().reset_index()
     delay_cat.columns = ['Delay Category', 'Count']
     delay_cat['Percentage'] = (delay_cat['Count'] / delay_cat['Count'].sum() * 100).round(2)
    ...
    query_carrier = "SELECT carrier, COUNT(*) as flight_count FROM flights GROUP BY carrier"
    ...
    df_times["dep_time_dt"] = df_times.apply(lambda r: convert_to_datetime(r, "dep_time"), axis=1)
    df_times["arr_time_dt"] = df_times.apply(lambda r: convert_to_datetime(r, "arr_time"), axis=1)

    df_times["dep_hour"] = df_times["dep_time_dt"].dt.hour
    df_times["arr_hour"] = df_times["arr_time_dt"].dt.hour
    df_times["flight_date"] = df_times.apply(lambda r: pd.Timestamp(year=int(r["year"]), month=int(r["month"]), day=int(r["day"])),
    axis=1)

    df_dep_counts = df_times.dropna(subset=["dep_hour"]).groupby(["flight_date","dep_hour"]).size().reset_index(name="dep_count")
    df_arr_counts = df_times.dropna(subset=["arr_hour"]).groupby(["flight_date","arr_hour"]).size().reset_index(name="arr_count")

    df_dep_counts = df_dep_counts.rename(columns={"dep_hour":"hour"})
    df_arr_counts = df_arr_counts.rename(columns={"arr_hour":"hour"})

    df_combined = pd.merge(df_dep_counts, df_arr_counts, on=["flight_date","hour"], how="outer")
    ...
    stats = df_combined.groupby("hour")[["dep_count","arr_count","total"]].agg(["mean","std"])
    ...
    stats["Time_Label"] = stats["Hour"].apply(lambda h: f"{int(h):02d}:00 - {int(h):02d}:59")

    fig = go.Figure()
    st.markdown("##### **Mean Hourly Numbers of Flight Departures and Arrivals (with Std. Dev.)**")
    fig.add_trace(
        go.Bar(
            x=stats["Time_Label"],
